...
local function GetPreviewOwner(unitType)
    if not ns.frames then return nil end
    if unitType == "party" then return ns.partyFrameMoverOwner or ns.frames.party1 end
    if unitType == "boss" then return ns.frames.boss1 end
    return nil
end
...
function ns.PreviewGroupLayout(unitType, layoutOverride, sizeOverride)
    ...
    local owner = GetPreviewOwner(unitType)
    if not owner then
        HideGroupPreview(unitType)
        return
    end
    ...
    if layoutOverride then for key, value in pairs(layoutOverride) do layout[key] = value end end
    previewLayouts[unitType] = layout

    local savedSize = ns.GetSize(unitType) or { width = 200, height = 40 }
    local width = sizeOverride and sizeOverride.width or owner:GetWidth() or savedSize.width
    local height = sizeOverride and sizeOverride.height or owner:GetHeight() or savedSize.height
    width = tonumber(width) or savedSize.width
    height = tonumber(height) or savedSize.height
...
function ns.ApplyPartyLayout()
    ...
    if not IsInGroup() or IsInRaid() then
        local owner = ns.frames.party1
        if owner then
            AnchorGroupFrames({ owner }, layout, "party1")
        ...
        if ns.SetFrameMoversLocked and ns.AreFrameMoversLocked then ns.SetFrameMoversLocked(ns.AreFrameMoversLocked()) end
        if ns.SetAuraMoversLocked and ns.AreAuraMoversLocked then ns.SetAuraMoversLocked(ns.AreAuraMoversLocked()) end
        return
    end
    ...
    ns.partyFrameMoverOwner = owner or ns.frames.party1
    ns.partyAuraMoverOwner = owner or ns.frames.party1
...
function ns.ApplyBossLayout()
    ...
    if ns.UpdateGroupPreviews and ns.AreFrameMoversLocked then ns.UpdateGroupPreviews(ns.AreFrameMoversLocked()) end
end
...
-- Keep the preview synchronized with the existing mover lock system without
-- making Layout.lua responsible for group-specific presentation.
